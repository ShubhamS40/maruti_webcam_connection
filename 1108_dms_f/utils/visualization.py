...
def draw_debug_overlay(frame, dbg):
    ...
    lines = [
        ...
        f"EAR baseline {dbg.get('baseline_ear', 0):.3f}",
        f"EAR close/open {dbg.get('ear_close_thr', 0):.3f}/{dbg.get('ear_open_thr', 0):.3f}",
        f"eye state    {dbg.get('eye_state', 'OPEN')}",
        f"closed sec   {dbg.get('eye_closed_sec', 0):.2f}",
        f"blinks       {dbg.get('blink_count', 0)}",
        f"PERCLOS n={dbg.get('perclos_n', 0)}  {dbg.get('perclos', 0):.3f}",
        f"raw MAR      {dbg.get('raw_mar', 0):.3f}",
        f"smooth MAR   {dbg.get('mar', 0):.3f}",
        f"MAR baseline {dbg.get('baseline_mar', 0):.3f}",
        f"yawn start/end {dbg.get('yawn_start_thr', 0):.2f}/{dbg.get('yawn_end_thr', 0):.2f}",
        f"yawn peak    {dbg.get('yawn_peak_mar', 0):.3f}",
        f"cand/need    {dbg.get('yawn_candidate_frames', 0)}/{dbg.get('yawn_required_frames', 0)}",
        f"yawn dur     {dbg.get('yawn_duration', 0.0):.2f}s",
        f"yawn conf    {dbg.get('yawn_confidence', 0):.2f}",
        f"yawn state   {dbg.get('yawn_state', 'NO_YAWN')}",
        f"yawn cool    {dbg.get('yawn_cooldown', 0)}",
        f"raw/sm risk  {dbg.get('raw_risk', 0):.3f}/{dbg.get('smooth_risk', 0):.3f}",
        f"CNN raw/sm   {dbg.get('cnn_raw', 0):.3f}/{dbg.get('cnn_smooth', 0):.3f}",
        f"pitch/base   {dbg.get('pitch', 0):.1f}/{dbg.get('pitch_baseline', 0):.1f}",
        f"state F/D/M/E {dbg.get('fatigue_frames', 0)}/{dbg.get('drowsy_frames', 0)}/{dbg.get('micro_frames', 0)}/{dbg.get('exit_frames', 0)}",
    ]

    for i, line in enumerate(lines):
        cv2.putText(
            frame, line, (x0, y0 + i * 18),
